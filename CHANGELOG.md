...
- Issue #33: model-validation diagnosis in the training pipeline (baseline-grounded underfitting signal, fold-variability overfitting and generalization signals, conditional improvement actions) persisted in the training report with unit tests.
- Issue #34: standalone batch inference pipeline script (new-data file → schema validation → artifact-driven prediction → predictions persisted to the model-output layer) with unit tests using a dummy model and synthetic data.
...
- Issue #51: remediated the seven vulnerable transitive dependencies (click, requests, idna, gitpython, filelock, pygments, virtualenv) and removed the temporary audit-ignore block introduced by Issue #49; `uv audit --locked` now passes with zero known vulnerabilities and zero ignored advisories.
