...
The Issue #8/#14 demo requirement provides a local Streamlit app for the
...
2. Upload a CSV matching the seven-column schema above.
3. Click **Run batch prediction**. The uploaded rows are validated against
   the same inference validation rules used by the batch pipeline and
   predictions are generated by the fitted artifact.
4. Preview the uploaded records with their `predicted_chance_of_admit`
   column.
5. Click **Download predictions (CSV)** to download the input rows plus the
   prediction column (appended last).
